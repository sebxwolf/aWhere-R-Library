#' @title GetWeatherNormsLatLon.
#'
#' @description
#' \code{GetWeatherNormsLatLon} calls Historic Agronomic Norms by Geolocation Endpoint of API using Lat/Lon
#'
#' @details
...
#'
#' @examples
#' GetWeatherNormsLatLon('39.8282', '-98.5795', '07-01', '07-10', '2008', '2015','2010,2011')

#' @export


GetWeatherNormsLatLon <- function(latitude, longitude, monthDayStart, monthDayEnd = '', yearStart, yearEnd, excludeYears = '') {
  ...
  if (yearStart != '' & yearEnd != '') {
    ...
    address <- paste0(urlAddress, strBeg, strCoord, strType, strMonthsDays, strYearsType,strYears,excludeYears)
  } else {
    address <- paste0(urlAddress, strBeg, strCoord, strType, strMonthsDays,excludeYears)
  }
  doWeatherGet <- TRUE
